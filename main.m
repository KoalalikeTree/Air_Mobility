...
params = struct(...
    ...
    'motor_constant',        36.5, ...
    'rpm_min',               3000, ... % you can increase this
    'rpm_max',               20000, ... % you can increase this
...
for iter = 1:max_iter-1
    ...
    % don't know why the first position is always zero
    if question == 8 || question ==9 || question == 2 || question == 5 || question == 6.1 || question == 6.2 || question == 6.3
        if iter == 1
            actual_desired_state_matrix(1:3,iter) =  desired_state.pos;
            actual_desired_state_matrix(4:6, iter) = desired_state.vel;
            actual_desired_state_matrix(7:9, iter) = desired_state.rot;
            actual_desired_state_matrix(10:12, iter) = desired_state.omega;
            actual_desired_state_matrix(13:15, iter) = desired_state.acc;
        end
        if iter == 2001
            actual_desired_state_matrix(1:15,iter) = actual_desired_state_matrix(1:15,iter-1);
        end
    end
...
if question == 8 || question == 9
    cum_error_matrix = cumsum(abs(actual_desired_state_matrix - actual_state_matrix),2);
    figure;
    subplot(3,5,1)
    plot(time_vec,cum_error_matrix(1,:))
    title('cum error in x');
    subplot(3,5,2)
    plot(time_vec,cum_error_matrix(2,:))
    title('cum error in y');
    subplot(3,5,3)
    plot(time_vec,cum_error_matrix(3,:))
    title('cum error in z');
    subplot(3,5,4)
    plot(time_vec,cum_error_matrix(4,:))
    title('cum error in x_dot');
    subplot(3,5,5)
    plot(time_vec,cum_error_matrix(5,:))
    title('cum error in y_dot');
    subplot(3,5,6)
    plot(time_vec,cum_error_matrix(6,:))
    title('cum error in z_dot');
    subplot(3,5,7)
    plot(time_vec,cum_error_matrix(7,:))
    title('cum error in phi');
    subplot(3,5,8)
    plot(time_vec,cum_error_matrix(8,:))
    title('cum error in theta');
    subplot(3,5,9)
    plot(time_vec,cum_error_matrix(9,:))
    title('cum error in psi');
    subplot(3,5,10)
    plot(time_vec,cum_error_matrix(10,:))
    title('cum error in phi_dot');
    subplot(3,5,11)
    plot(time_vec,cum_error_matrix(11,:))
    title('cum error in theta_dot');
    subplot(3,5,12)
    plot(time_vec,cum_error_matrix(12,:))
    title('cum error in psi_dot');
    subplot(3,5,13)
    plot(time_vec,cum_error_matrix(13,:))
    title('cum error in x_acc');
    subplot(3,5,14)
    plot(time_vec,cum_error_matrix(14,:))
    title('cum error in y_acc');
    subplot(3,5,15)
    plot(time_vec,cum_error_matrix(15,:))
    title('cum error in z_acc');
end

%% get the risetime/overshoot/... value

z_position = actual_state_matrix(3,:);
z_velocity = actual_state_matrix(6,:);
yaw = actual_state_matrix(9,:);
t = time_vec;
z_position_perform = stepinfo(z_position,t,'SettlingTimeThreshold',0.005,'RiseTimeThreshold',[0 0.9])
z_velocity_perform = stepinfo(z_velocity,t,'SettlingTimeThreshold',0.005,'RiseTimeThreshold',[0 0.9])
yaw_perform = stepinfo(yaw,t,'SettlingTimeThreshold',0.005, 'RiseTimeThreshold',[0 0.9])
